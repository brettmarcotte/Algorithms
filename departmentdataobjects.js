...
function departmentData() {
  ...
  for (let i = 0; i < departments.length; i++) {
    ...
    for (let j = 0; j < employeeDepartment.length; j++) {
      const emplDept = employeeDepartment[j];

      if (dept[0] === emplDept[1] && emplDept[3] == "9999-01-01") {

        for (let k = 0; k < employees.length; k++) {
          const empl = employees[k];

          if (emplDept[0] == empl[0]) {

            total++
          }
          for (let l = 0; l < employees.length; l++) {
            const emplData = employees[4];
                if(emplData[1] == emplDept 
                  && isCurrentlyEmployed(emplData[0])
                  ) {
                  if( isMaleEmployee(emplData[0]) ) {
                  genders.male++
              } else {
                  genders.female++
              }
    ...
    let deptData = { id: dept[0], names: dept[1], totalEmployees: total, gender: employee[l][4] }

    departmentInfo.push(deptData)
  }
...
function employeeData() {
  ...
  for (let i = 0; i < employees.length; i++) {
    ...
    for (let j = 0; j < employeeDepartment.length; j++) {
      const emplDept = employeeDepartment[j][1]

      if (emplDept === empl[1] && empl[3] == "9999-01-01")
      
      for (let k = 0; k < employeeSalaries.length; k++) {
        const emplSal = employeeSalaries[k][1];
        
        if (emplSal[0] === emplDept[1] && emplDept[3] == "9999-01-01")
       
        let workerInfo = { id: employees[i][0], name: employees[i][2] + " " + employees[i][3], gender: employees[i][4], birthday: employees[i][1], hireDate: employees[i][5], isStillEmployed: employeeDepartment[j][3], departmentsWorkedFor: employeeDepartment[j][1], mostRecentSalaries: employeeSalaries[k][1]}
      }
